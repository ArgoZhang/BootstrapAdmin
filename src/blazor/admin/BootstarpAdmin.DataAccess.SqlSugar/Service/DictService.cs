...
class DictService(ISqlSugarClient db, IConfiguration configuration) : IDict
{
    ...
    {
        //避免多次连续不间断加载出错。先释放
        var dicts = db.Queryable<Dict>().ToList();
        return dicts;
    });
    ...
    {
        var ret = db.Updateable<Dict>().Where(s => s.Category == dict.Category && s.Name == dict.Name).SetColumns(s => s.Code, dict.Code).ExecuteCommand() > 0;
        if (ret)
        {
            // 更新缓存
    ...
    public bool SaveClient(ClientApp client)
    {
        ...
        if (!string.IsNullOrEmpty(client.AppId))
        {
            ...
            try
            {
                ...
                ret = db.Insertable(items).ExecuteCommand() > 0;
            }
            catch
            {
    ...
    public bool DeleteClient(string appId)
    {
        ...
        try
        {
            ...
            //db.Ado.ExecuteCommand("delete from Dicts where Category=@Category and Name=@Name and Define=@Define", new { Category = "应用首页", Name = appId, Define = EnumDictDefine.System });
            //db.Ado.ExecuteCommand("delete from Dicts where Category=@Category and Code=@Code and Define=@Define", new { Category = "应用程序", Code = appId, Define = EnumDictDefine.System });
            //db.Ado.ExecuteCommand("delete from Dicts where Category=@Category and Name in (@Names)", new
            //{
            //    Category = appId,
            ...
            //改linq写法
            db.Deleteable<Dict>().Where(t => t.Category == "应用首页" && t.Name == appId && t.Define == EnumDictDefine.System).ExecuteCommand();
            db.Deleteable<Dict>().Where(t => t.Category == "应用程序" && t.Code == appId && t.Define == EnumDictDefine.System).ExecuteCommand();
            ...
            db.Deleteable<Dict>().Where(t => t.Category == appId && names.Contains(t.Name)).ExecuteCommand();
